...
+ (NSImage *)qrImageWithContent:(NSString *)content size:(CGFloat)size {
    CIImage *image = [self qrByContent:content];
...
+ (NSImage *)qrImageWithContent:(NSString *)content size:(CGFloat)size color:(NSColor *)color {
    return [self qrImageWithContent:content size:size red:color.redComponent green:color.greenComponent blue:color.blueComponent];
}
...
+ (NSImage *)qrImageWithContent:(NSString *)content size:(CGFloat)size red:(NSInteger)red green:(NSInteger)green blue:(NSInteger)blue {
    NSImage *image = [self qrImageWithContent:content size:size];
    ...
    CGContextDrawImage(context, CGRectMake(0, 0, imageWidth, imageHeight), [self nsImageToCGImageRef:image]);
    // 遍历像素
    ...
    NSImage *resultUIImage = [self imageFromCGImageRef:imageRef];
    // 清理空间
...
+ (NSImage *)qrImageWithContent:(NSString *)content logo:(NSImage *)logo size:(CGFloat)size color:(NSColor *)color {
    return [self qrImageWithContent:content logo:logo size:size red:color.redComponent green:color.greenComponent blue:color.blueComponent];
}
